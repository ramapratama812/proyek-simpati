...
@section('content')
    <div class="container-fluid py-4">
        ...
        <div class="card header-main-card mb-5 shadow-lg border-0 rounded-4 overflow-hidden">
            <div class="card-body p-0 bg-primary-gradient text-white position-relative">
                ...
                <div class="header-title-area py-4 px-4">
                    <h1 class="fs-4 mb-1 fw-bold text-uppercase position-relative z-1"><i
                            class="bi bi-mortarboard-fill me-2"></i> DATA DOSEN TEKNOLOGI INFORMASI</h1>
                    <p class="mb-0 small opacity-75 position-relative z-1 header-subtitle-text">Manajemen dan informasi
                        seluruh data dosen.</p>
                ...
                <div class="row m-0 stat-row-integrated px-4 pb-4">
                    ...
                    <div class="col-md-4 col-sm-6 p-2">
                        <div
                            class="stat-box-integrated hover-smooth-shadow shadow-sm text-primary-dark rounded-3 p-3 text-center">
        ...
        <div class="card p-4 mb-4 shadow-md border-0 rounded-4 filter-card">
            <h6 class="fw-bold text-primary-dark mb-3 border-bottom pb-2"><i class="bi bi-funnel-fill me-2"></i> PENCARIAN &
                FILTER DATA</h6>
            <form action="{{ route('dosen.index') }}" method="GET" class="d-flex flex-wrap align-items-end gap-3">
                ...
                <div class="flex-grow-1" style="min-width: 250px; max-width: 450px;">
                    ...
                    <div
                        class="input-group search-input-group rounded-pill overflow-hidden border border-2 border-light-subtle">
                        <input id="search_input" type="text" name="search" value="{{ $search ?? '' }}"
                            class="form-control ps-4 border-0" placeholder="Masukkan kata kunci...">
                ...
                <div style="width: 180px;">
                    ...
                    <select id="sort_select" name="sort" class="form-select filter-select shadow-sm"
                ...
                <div style="width: 180px;">
                    ...
                    <select id="status_select" name="status" class="form-select filter-select shadow-sm"
                        onchange="this.form.submit()">
                ...
                @if (request()->hasAny(['search', 'sort', 'status']))
                    <div class="ms-md-auto">
                        <a href="{{ route('dosen.index') }}"
                            class="btn btn-outline-secondary rounded-pill px-3 py-2 small">
        ...
        <div class="card table-container shadow-lg rounded-4 border-0">
            <div class="card-header bg-white border-0 pt-4 pb-0 d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <p class="text-muted small mb-0">Menampilkan {{ $dosens->count() }} hasil dari total
                        {{ $dosens->total() ?? 0 }} data.</p>
                </div>
            </div>
                                ...
                                <tr class="table-row">
                                    ...
                                    <td>
                                        ...
                                        @elseif ($dosen->status_aktivitas == 'Tidak Aktif')
                                            <span class="badge status-tidak-aktif"><i class="bi bi-x-circle-fill me-1"></i>
                                                Non Aktif</span>
                                        @elseif ($dosen->status_aktivitas == 'Cuti')
                                            <span class="badge status-cuti"><i class="bi bi-pause-circle-fill me-1"></i>
                                                Cuti</span>
                                        @else
                                    ...
                                    <td class="text-center">
                                        <a href="{{ route('dosen.show', $dosen->id) }}"
                                            class="btn btn-sm btn-detail fw-semibold">
