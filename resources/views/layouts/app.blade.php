    ...
    <style>
        * { 
        ...
        main {
            ...
            animation: fadeIn 0.4s ease-out; /* Ditingkatkan durasinya */
        }

        @keyframes fadeIn {
            from { opacity: 0; transform: translateY(10px); } /* Jarak start lebih besar */
            ...
            <div class="sidebar">
                ...
                @auth
                    @php 
                        ...
                        if ($role === 'dosen' && method_exists($user, 'dosen')) {
                            $modelData = $user->dosen()->first();
                            if ($modelData && isset($modelData->foto) && $modelData->foto) {
                                $user_photo_path = $modelData->foto;
                            }
                        } elseif ($role === 'mahasiswa' && method_exists($user, 'mahasiswa')) {
                            $modelData = $user->mahasiswa()->first();
                            if ($modelData && isset($modelData->foto) && $modelData->foto) {
                                $user_photo_path = $modelData->foto;
                            }
                        }
                        ...
                        @auth
                            ...
                            <ul class="navbar-nav align-items-center">
                                ...
                                <li class="nav-item dropdown">
                                    <a class="nav-link profile-avatar-container"
                                        href="#" 
                                        id="profileDropdown"
                                        role="button"
                                        data-bs-toggle="dropdown"
                                        ...
                                        @else
                                            <div class="rounded-circle d-flex align-items-center justify-content-center fw-bold navbar-initial">
                                                {{ $initial }}
                                            </div>
                                        @endif
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end mt-2 shadow-sm border-0"
                                        ...
                                        <li>
                                            <a class="dropdown-item d-flex align-items-center"
                                                ...
                                                <i class="bi bi-person-circle me-2"></i> Lihat Profil ({{ $user->name }})
                                            </a>
                                        </li>
