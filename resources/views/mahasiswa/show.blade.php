...
{{-- 
    👇 PERBAIKAN LOGIKA LINK PDDIKTI (TAMBAH SLASH) 👇 
--}}
@php
    $pddiktiUrl = ($mahasiswa->nim ?? false) 
        ...
        <div class="card-body px-4 px-md-5 py-5" style="background-color: #fbfbff;">
            ...
            <div class="scroll-list-container mb-5">
                ...
                @forelse($memberships as $member)
                    @if($member->project)
                        <a href="{{ route('projects.show', $member->project->id) }}" class="text-decoration-none text-dark d-block">
                                ...
                                <div class="d-flex justify-content-between align-items-start">
                                    <div>
                                        ...
                                        <div class="text-muted small">
                                            ...
                                            <span class="fw-semibold text-dark">{{ ucfirst($member->role ?? 'Anggota') }}</span>
                                            <span class="mx-2">|</span>
                                            <i class="bi bi-calendar-event me-1"></i> Tahun: {{ $member->project->tanggal ?? $member->project->tahun_usulan ?? '-' }}
                                        </div>
                                    </div>
                                    <span class="badge bg-light text-secondary border">Detail <i class="bi bi-chevron-right ms-1"></i></span>
                                </div>
                            </div>
                        </a>
                    @endif
            ...
            <div class="d-flex justify-content-between align-items-center">
                ...
                {{-- TOMBOL KEMBALI --}}
                <a href="{{ route('mahasiswa.index') }}" class="btn btn-secondary rounded-pill px-4 btn-smooth-action btn-back">
                    <i class="bi bi-arrow-left me-1"></i> Kembali
                </a>

                {{-- ❗ BLOK TOMBOL EDIT & HAPUS (HANYA MUNCUL DI profile.show) ❗ --}}
                @if (isset($isProfileView) && $isProfileView)
                    <div class="d-flex gap-2">
                        ...
                        <form action="{{ $deleteRoute }}" method="POST"
                              onsubmit="return confirm('Yakin ingin hapus akun ini? Data tidak bisa dikembalikan.')" class="d-inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
...
/* Global Transition */
.btn-smooth-action, .bio-box, .pddikti-box a.btn, .kegiatan-box {
...
/* 2. Avatar Styling */
.dosen-avatar-in-header { display: inline-block; }
...
.bio-box i { align-self: flex-start; margin-bottom: 5px; }
.bio-box:hover {
...
/* Status Badges */
.badge { font-size: 0.8em; padding: 0.5em 0.8em; border-radius: 50px; font-weight: 600; }
...
.btn-edit-profile {
    ...
    border: 1px solid var(--primary-dark); box-shadow: 0 4px 10px rgba(0, 80, 160, 0.2);
}
...
.btn-delete-account { border-color: var(--danger-color); color: var(--danger-color) !important; }
.btn-delete-account:hover {
    background-color: var(--danger-color); color: white !important;
    transform: translateY(-2px); box-shadow: 0 6px 15px rgba(220, 53, 69, 0.4);
}

.btn-back:hover { transform: translateY(-1px); box-shadow: 0 3px 8px rgba(0, 0, 0, 0.15); }
...
.scroll-list-container::-webkit-scrollbar-track { background-color: #f1f1f1; }
</style>
@endsection